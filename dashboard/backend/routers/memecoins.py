...
@router.get("/outcome-audit")
async def memecoin_outcome_audit_ep(window_days: int = 90, _: str = Depends(get_current_user)):
    """
    Group closed memecoin trade outcomes by entry-time score state so we can
    evaluate whether the new readiness-first model is learning anything useful.
    """
    def _run():
        from utils.db import get_memecoin_outcome_audit  # type: ignore
        return get_memecoin_outcome_audit(window_days=max(7, int(window_days)))

    try:
        return await asyncio.to_thread(_run)
    except Exception as exc:
        log.warning("memecoin_outcome_audit_ep error: %s", exc)
        return {
            "summary": {"window_days": max(7, int(window_days)), "closed_trades": 0, "attributed_trades": 0},
            "groups": {},
            "review_drift": {},
            "recent_examples": [],
        }
...
@router.get("/calibration-view")
async def memecoin_calibration_view_ep(window_days: int = 90, limit: int = 25, _: str = Depends(get_current_user)):
    """
    Compact learning view for the new timing/latency/exit fields so we can audit
    whether the system is entering too late and capturing enough of the move.
    """
    def _run():
        from utils.db import get_memecoin_calibration_view  # type: ignore
        return get_memecoin_calibration_view(
    ...
    try:
        ...
        log.warning("memecoin_calibration_view_ep error: %s", exc)
        return {
...
def get_proof_stack_summary_data(limit: int = 25) -> dict:
    ...
    for row in trade_rows:
        ...
        readiness = _v3_readiness_from_snapshot(parsed)
        trade_item = {
            "id": int(row["id"]),
    ...
    with sqlite3.connect(_db_path()) as conn:
        ...
        for row in scanner_rows:
            ...
            candidate_rows.append({
                ...
                "wallet_reinforcement_level": str(proof_candidate_row.get("wallet_reinforcement_level") or "NONE"),
                "wallet_overlap_score": float(proof_candidate_row.get("wallet_overlap_score") or 0.0),
                "wallet_confidence": float(proof_candidate_row.get("wallet_confidence") or 0.0),
                "wallet_unique_wallets": int(proof_candidate_row.get("wallet_unique_wallets") or 0),
                "wallet_repeat_wallets": int(proof_candidate_row.get("wallet_repeat_wallets") or 0),
                "wallet_high_quality_wallets": int(proof_candidate_row.get("wallet_high_quality_wallets") or 0),
                "wallet_recent_activity": bool(proof_candidate_row.get("wallet_recent_activity")),
                "wallet_reasons": list(proof_candidate_row.get("wallet_reasons") or [])[:4],
...
def get_memecoin_graduation_data(limit: int = 8, lookback_days: int = 30) -> dict:
    ...
    proof_ready_now = int(pipeline.get("proof_ready_now") or 0)

    mode = str(get_memecoin_mode() or "PAPER").upper()
    auto_buy = os.getenv("MEMECOIN_AUTO_BUY", "false").lower() == "true"
    bootstrap_enabled = os.getenv("MEMECOIN_PILOT_PROOF_BOOTSTRAP", "false").lower() == "true"
    if not auto_buy:
        ...
        and proof_ready_now > 0
        and used_slots < max(recommended_slots, 1)
    ):
        deployment_authority = "LIMITED"
        active_policy_posture = "PILOT_BOOTSTRAP_PROBE"
    ...
    policy_recommendation = {
        ...
        "deployment_recommendation": (
            ...
            "Pilot bootstrap is enabled for a single strict proof-build probe."
            if active_policy_posture == "PILOT_BOOTSTRAP_PROBE" else
...
def _v3_support_signal(tag: str, detail: str = "") -> dict:
    ...
    labels = {
        ...
        "wallet_backed": "Wallet Backed",
        "wallet_repeat_overlap": "Repeat Wallet Flow",
        "wallet_high_quality_support": "High-Quality Wallets",
        "wallet_recent_activity": "Fresh Wallet Activity",
    }
    severities = {
        "whale_overlap": "GOOD",
        ...
        "wallet_backed": "GOOD",
        "wallet_repeat_overlap": "GOOD",
        "wallet_high_quality_support": "GOOD",
        "wallet_recent_activity": "INFO",
    }
...
def _build_v3_candidate(candidate: dict, proof_row: dict | None, graduation: dict, pipeline: dict) -> dict:
    ...
    readiness_block = dict(proof_components.get("readiness") or {})
    ...
    wallet_block = dict(reinforcement_block.get("wallet") or {})
    wallet_level = str(candidate.get("wallet_reinforcement_level") or wallet_block.get("wallet_support_level") or "NONE")
    wallet_reasons = [
        _v3_support_signal("wallet_backed", str(reason or ""))
        for reason in list(candidate.get("wallet_reasons") or wallet_block.get("reasons") or [])[:4]
    ...
    return {
        ...
        "scores": {
            "timing": candidate.get("timing_score") or scanner_block.get("timing_score"),
            "safety": candidate.get("safety_score") or scanner_block.get("safety_score"),
            "market": candidate.get("market_quality_score") or scanner_block.get("market_quality_score"),
            "support": candidate.get("support_score") or reinforcement_block.get("support_score"),
            "readiness": candidate.get("readiness_score") or readiness_block.get("readiness_score"),
            "profit_room": candidate.get("profit_room_score") or readiness_block.get("profit_room_score") or scanner_block.get("profit_room_score"),
        },
        "profit_room": {
            "label": str(
                candidate.get("profit_room_label")
                or readiness_block.get("profit_room_label")
                or scanner_block.get("profit_room_label")
                or ""
            ) or None,
            "reasons": [
                _v3_reason(str(reason or ""), _v3_reason_label(str(reason or "")))
                for reason in list(
                    candidate.get("profit_room_reasons")
                    or readiness_block.get("profit_room_reasons")
                    or scanner_block.get("profit_room_reasons")
                    or []
                )[:4]
                if str(reason or "").strip()
            ],
        ...
        "trade_quality": {
            "verdict": str(
                candidate.get("market_quality_verdict")
                or scanner_block.get("market_quality_verdict")
                or ""
            ) or None,
            "reasons": [
                _v3_reason(str(reason or ""), _v3_reason_label(str(reason or "")))
                for reason in list(
                    candidate.get("market_quality_reasons")
                    or scanner_block.get("market_quality_reasons")
                    or []
                )[:4]
                if str(reason or "").strip()
            ],
        ...
        "reinforcement": {
            ...
            "score": float(candidate.get("reinforcement_score") or 0.0),
            "support_score": float(candidate.get("support_score") or reinforcement_block.get("support_score") or 0.0),
            "support_level": str(candidate.get("support_level") or reinforcement_block.get("support_level") or "NONE"),
            "support_kind": support_kind,
            "confidence": {
                "EXACT_MINT": 0.95,
                "SYMBOL_FAMILY": 0.6,
            ...
            "wallet": {
                "level": wallet_level,
                "overlap_score": float(candidate.get("wallet_overlap_score") or wallet_block.get("wallet_overlap_score") or 0.0),
                "confidence": float(candidate.get("wallet_confidence") or wallet_block.get("wallet_confidence") or 0.0),
                "unique_wallets": int(candidate.get("wallet_unique_wallets") or wallet_block.get("unique_wallets") or 0),
                "repeat_wallets": int(candidate.get("wallet_repeat_wallets") or wallet_block.get("repeat_wallets") or 0),
                "high_quality_wallets": int(candidate.get("wallet_high_quality_wallets") or wallet_block.get("high_quality_wallets") or 0),
                "recent_activity": bool(candidate.get("wallet_recent_activity") or wallet_block.get("recent_wallet_activity")),
        ...
        "proof": {
            ...
            "readiness_score": candidate.get("readiness_score") or readiness_block.get("readiness_score"),
            "readiness_level": candidate.get("readiness_level") or readiness_block.get("readiness_level"),
            "readiness_reasons": list(readiness_block.get("readiness_reasons") or []),
            "hard_blockers": hard_blockers,
            "soft_penalties": soft_penalties,
...
def _v3_readiness_from_snapshot(snapshot: dict | None) -> dict:
    proof_components = dict((snapshot or {}).get("proof_components") or {})
    readiness = dict(proof_components.get("readiness") or {})
    return {
        "readiness_score": readiness.get("readiness_score"),
        "readiness_level": readiness.get("readiness_level"),
        "readiness_reasons": list(readiness.get("readiness_reasons") or []),
    }


def _v3_review_state_label(review_state: object) -> str | None:
    state = str(review_state or "").strip().upper()
    if not state:
        return None
    mapping = {
        "HOLD": "hold",
    ...
    return mapping.get(state, state.replace("_", " ").lower())


def _v3_exit_reason_label(exit_reason: object) -> str | None:
    raw = str(exit_reason or "").strip()
    if not raw:
        return None
    key, _, detail = raw.partition(" — ")
    mapping = {
        "readiness_fade": "readiness weakened while trade stayed green",
        "readiness_fade_loss": "readiness weakened while trade slipped red",
        "legacy_proof_fade": "legacy proof signal weakened while trade stayed green",
        "legacy_proof_fade_loss": "legacy proof signal weakened while trade slipped red",
        "tier_one_take_profit": "first take-profit target hit",
        "tier_two_take_profit": "second take-profit target hit",
        "time_target_hit": "time-based profit target hit",
        "stale_winner": "winner aged out of the hold window",
        "stop_loss": "stop loss hit",
        "rug_emergency": "emergency rug exit",
    }
    base = mapping.get(key, key.replace("_", " ").lower())
...
def get_memecoin_v3_candidate_detail(mint: str, lookback_days: int = 30) -> dict:
    ...
    with sqlite3.connect(_db_path()) as conn:
        ...
        history_rows = conn.execute(
            ...
            SELECT
                ...
                proof_score, proof_reason, proof_snapshot_json, return_4h_pct, return_24h_pct,
        ...
        for row in history_rows:
            row_data = dict(row)
            try:
                parsed_snapshot = json.loads(str(row_data.get("proof_snapshot_json") or "")) if row_data.get("proof_snapshot_json") else {}
                if not isinstance(parsed_snapshot, dict):
                    parsed_snapshot = {}
            except Exception:
                parsed_snapshot = {}
            readiness = _v3_readiness_from_snapshot(parsed_snapshot)
    ...
    if open_trade is not None:
        ...
        if isinstance(open_trade.get("exit_review"), dict):
            _review = dict(open_trade["exit_review"])
            _review["review_state_label"] = _v3_review_state_label(_review.get("review_state"))
            _review["exit_reason_label"] = _v3_exit_reason_label(_review.get("exit_reason"))
            open_trade["exit_review"] = _review
        if isinstance(open_trade.get("exit_history"), list):
            _hist = []
            for _row in list(open_trade.get("exit_history") or []):
                _item = dict(_row or {})
                _item["review_state_label"] = _v3_review_state_label(_item.get("review_state"))
                _item["exit_reason_label"] = _v3_exit_reason_label(_item.get("exit_reason"))
                _hist.append(_item)
            open_trade["exit_history"] = _hist
    recent_outcomes = []
...
def get_memecoin_v3_proof_workspace(limit: int = 12, lookback_days: int = 30) -> dict:
    ...
    for item in list(proof_review.get("open_trades") or []):
        ...
        if isinstance(trade.get("exit_review"), dict):
            _review = dict(trade["exit_review"])
            _review["review_state_label"] = _v3_review_state_label(_review.get("review_state"))
            _review["exit_reason_label"] = _v3_exit_reason_label(_review.get("exit_reason"))
            trade["exit_review"] = _review
        if isinstance(trade.get("exit_history"), list):
            _hist = []
            for _row in list(trade.get("exit_history") or []):
                _item = dict(_row or {})
                _item["review_state_label"] = _v3_review_state_label(_item.get("review_state"))
                _item["exit_reason_label"] = _v3_exit_reason_label(_item.get("exit_reason"))
                _hist.append(_item)
            trade["exit_history"] = _hist
    ...
    for item in list(proof_review.get("recent_outcomes") or [])[: max(int(limit), 1)]:
        ...
        recent_outcomes.append({
            ...
            "readiness_score": outcome.get("readiness_score"),
            "readiness_level": outcome.get("readiness_level"),
            "readiness_reasons": list(outcome.get("readiness_reasons") or []),
    ...
    return {
        ...
        "recent_blocker_summary": _v3_recent_blocker_summary(hours=24, limit=120),
    }
...
def _v3_recent_blocker_summary(hours: int = 24, limit: int = 120) -> dict:
    """Summarize dominant recent scanner blockers from persisted scanner outcomes."""
    try:
        with sqlite3.connect(_db_path()) as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
                """
                SELECT symbol, score, rug_label, top_holder_pct, top5_holder_pct,
                       holder_quality_level, entry_context
                FROM memecoin_signal_outcomes
                WHERE source='SCANNER'
                  AND scanned_at >= datetime('now', ?)
                ORDER BY scanned_at DESC
                LIMIT ?
                """,
                (f"-{int(hours)} hours", max(10, int(limit))),
            ).fetchall()
    except Exception:
        rows = []
    ...
    def _push(key: str, symbol: str) -> None:
        bucket = categories.setdefault(key, {"key": key, "count": 0, "sample_symbols": []})
        bucket["count"] += 1
        if symbol and symbol not in bucket["sample_symbols"] and len(bucket["sample_symbols"]) < 4:
    ...
    for row in rows:
        ...
        rug = str(row["rug_label"] or "UNKNOWN").upper()
        top1 = float(row["top_holder_pct"] or 0.0)
        top5 = float(row["top5_holder_pct"] or 0.0)
        hq = str(row["holder_quality_level"] or "UNKNOWN").upper()
        ctx = str(row["entry_context"] or "UNKNOWN").upper()
        reviewed += 1

        if rug in ("DANGER", "RUGGED"):
            _push("rug_danger", symbol)
            continue
        if hq == "BLOCKED":
            _push("holder_blocked", symbol)
            continue
        if top1 > 8.0 or top5 > 40.0 or hq == "RISKY":
            _push("holder_risky", symbol)
            continue
        if score < 55.0:
            _push("score_too_low", symbol)
            continue
        if ctx in ("EXTENDED", "DISTRIBUTION_RISK", "DEAD_OR_CHOP"):
            _push("context_weak", symbol)
            continue
        if rug == "WARN":
            _push("rug_warn", symbol)
            continue
        _push("other", symbol)

    ordered = sorted(categories.values(), key=lambda item: item["count"], reverse=True)
    dominant = ordered[0]["key"] if ordered else None
    return {
        "window_hours": int(hours),
        "total_reviewed": reviewed,
        "dominant_key": dominant,
        "categories": ordered[:6],
    }
...
def _build_scanner_diagnostics_payload(limit: int = 10) -> dict:
    ...
    return scan_trending_solana_diagnostics(None, None, None, limit)
...
async def memecoins_scanner_diagnostics_ep(_: str = Depends(get_current_user), limit: int = 10):
    ...
    try:
        return await snapshot_or_build(
            f"memecoins:scanner-diagnostics:{int(limit)}",
            lambda: _build_scanner_diagnostics_payload(limit),
            fresh_s=60,
            stale_s=600,
            wait_timeout_s=4,
        )
        ...
        return {
            "stage_counts": {"returned_relaxed": 0, "returned_normal": 0},
            "top_scored": [],
            "error": f"scanner_diagnostics_warming:{exc}",
            "_snapshot": {
                "name": f"memecoins:scanner-diagnostics:{int(limit)}",
                "updated_at": None,
                "age_seconds": None,
                "status": "WARMING",
                "source": "fallback",
